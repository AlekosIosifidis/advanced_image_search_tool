...
def extract_face_information(images_folder):
    ...
    with open('data/faces.pickle', 'wb') as handle:
...
def extract_scene_information(images_folder):
    ...
    with open('data/environments.pickle', 'wb') as handle:
...
def extract_object_information(images_folder):
    ...
    with open('data/objects.pickle', 'wb') as handle:
...
def extract_image_captions(images_folder):
    ...
    with open('data/captions.pickle', 'wb') as handle:
...
def extract_main_character(images_folder):
    ...
    with open('data/main_character.pickle', 'wb') as handle:
...
def extract_gazes(images_folder):
    ...
    with open('data/gazes.pickle', 'wb') as handle:
...
def extract_date_information(images_folder):
    ...
    with open('data/date.pickle', 'wb') as handle:
...
def extract_dimension_information(images_folder):
    ...
    with open('data/dimensions.pickle', 'wb') as handle:
...
def extract_age_gender(images_folder):
    ...
    with open('data/age_gender.pickle', 'wb') as handle:
...
def extract_emotion(images_folder):
    ...
    with open('data/emotions.pickle', 'wb') as handle:
        pickle.dump(emotion_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

    return

def hue_to_color(hues):
    ...
    hues = hues.flatten()
    color_counts[0] += np.count_nonzero((175 < hues))
    color_counts[0] += np.count_nonzero((hues <= 6))#red
    color_counts[1] = np.count_nonzero((155 < hues) & (hues <= 175)) #pink
    color_counts[2] = np.count_nonzero((6 < hues) & (hues <= 23)) #orange
    color_counts[3] = np.count_nonzero((23 < hues) & (hues <= 32)) #yellow
    color_counts[4] = np.count_nonzero((32 < hues) & (hues <= 75)) #green
    color_counts[5] = np.count_nonzero((75 < hues) & (hues <= 90)) #green-blue
    color_counts[6] = np.count_nonzero((90 < hues) & (hues <= 123)) #blue
    color_counts[7] = np.count_nonzero((123 < hues) & (hues <= 155)) #purple

    if max(color_counts)/sum(color_counts) > 0.3:
...
def extract_color_information(images_folder):
    ...
    for filename in file_list:
        ...
        if not isGreyscale:
            ...
            h = hsv[:,:,0]
            dominantColor, dominantSecondColor = hue_to_color(h)
    ...
    with open('data/colors.pickle', 'wb') as handle:
